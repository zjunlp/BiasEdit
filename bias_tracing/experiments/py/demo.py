...
def demo_model_editing(
    ...
    params_name = (
        HPARAMS_DIR
        / hparams_prefix
        / f"{model.config._name_or_path.split('/')[-1].replace('/', '_')}{hparams_suffix}.json"
    )
    ...
    pre_update_text = generate_fast(model, tok, generation_prompts, max_out_len=100)    # generate text

...
class Trace(contextlib.AbstractContextManager):
    ...
    def __init__(
        ...
        def retain_hook(m, inputs, output):                     # wte: intputs[0] are input_ids (batch_size, seq_len), output is word embedding (batch_size, seq_len, emb_size)
            if retain_input:                                    # ffn: inputs[0]'s shape is (batch_size, seq_len, hidden_size), output is (batch_size, seq_len, emb_size)
...
def invoke_with_optional_args(fn, *args, **kwargs):
    ...
    those new arguments.
    """
    argspec = inspect.getfullargspec(fn)  # fn is patch_rep
    pass_args = []
    used_kw = set()
    unmatched_pos = []
    used_pos = 0
